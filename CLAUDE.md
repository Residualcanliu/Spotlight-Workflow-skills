...
**当前状态：v3.1，SKILL.md 为常驻核心指令（~10 KB），写作规范独立成 `WRITING.md`，其余参考见 `REFERENCE.md`（均按需读取）。写作铁律已扩至十一项，分词汇层与结构层两类。**
...
├── SKILL.md               ← 核心指令（~10 KB，每次加载）
├── WRITING.md             ← 写作铁律完整规范（十一项，按需读取）
├── REFERENCE.md           ← 参考手册（按需读取）
├── CLAUDE.md              ← 本文件
├── version.md             ← 版本迭代记录
├── spotlight-init/
