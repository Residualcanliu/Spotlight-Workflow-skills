...
**触发条件：** 任务复杂度判定为"复杂"（3+ 独立步骤 或 2+ 文件，见主技能 §2.4）。

**类比：** 数据库 WAL（连续日志）+ 定期全量备份 → 出问题可 PITR 回滚。

#### 日志文档（LOG.md）

项目启动时在项目根目录创建 `LOG.md`，边做边记，不等到最后写：

**记录内容：**
- 每个 TODO 开始/完成时写一条
- 关键决策 + 原因（"为什么选方案 A 而不是 B"）
- **运行输出和测试结果文本化** — 报错信息原文粘贴、关键输出截图用文字描述。让不支持多模态的 AI agent 也能通过读日志"看到"发生了什么
- 方向转变记录（"原计划 X，因为 Y 原因改为 Z"）

**格式示例：**
```markdown
# 项目日志 — {项目名称}

## 2026-07-12 14:00 — 项目启动
- 创建项目骨架，技术栈：React + Three.js
- 复杂度判定：复杂（预估 4 个独立模块）

## 2026-07-12 14:30 — TODO-1 环境搭建完成
- Vite + React 脚手架就位
- 运行 `npm run dev` 成功，无报错

## 2026-07-12 15:00 — [存档点 #1] 基础框架就绪
- 路由、状态管理、目录结构全部到位
- npm run build 通过
```
...
**设存档点的时机：**
- 完成一个主要 TODO 项后
- 高风险操作前（大重构、批量替换、依赖升级）
- 阶段性成果产出后（"模型训完了""核心功能跑通"）
- 当日工作结束前
...
2. **Git commit：**
   ```bash
   git add -A && git commit -m "savepoint: {简短描述}"
   ```
   Commit message 统一前缀 `savepoint:` 方便检索。

**回滚：** `git log --grep="savepoint:"` 列出所有存档点，`git reset --hard <commit>` 回滚到目标存档点。LOG.md 中可查看每个存档点的上下文。

#### 与复盘的关系

| | LOG.md（日志） | 复盘文件（learn/） |
|---|---|---|
| 时机 | 项目中持续写 | 项目结束后写 |
| 内容 | 发生了什么 | 学到了什么 |
| 粒度 | 步骤级（每一步） | 项目级（总结） |
| 读者 | AI agent（含非多模态）+ 人类 | 人类（未来参考） |
| 目的 | 过程追溯 + 回滚支持 | 经验沉淀 |

日志是复盘的**原材料**——写复盘时翻 LOG.md，不会遗漏关键细节。
...
**写作铁律的完整规范（十一条、逐条 [N]/[Y] 示例、真实改稿案例、自检三问）
已拆到同目录下的 `WRITING.md`，本文件不再重复。**

这里只留一条概要：铁律分两层，前六条管词汇与句式（括号参数、步骤回顾、
结果分段、「一句话」、「说人话就是」、「不是A而是B」），后五条管结构与语气
（名词短语标题、元话语、破折号、先声明数量、书面动词与正向结论）。

**为什么拆出去：** SKILL.md 要保持 ~10 KB 常驻，REFERENCE.md 也宜精简。
写作规范会被频繁查阅、且内容会持续增补，独立成文件更好维护。
...
| **整体概括** | 整篇风格的本质 | 粗粒度模块 + 第一人称 + 三段节奏 + 口语化过渡 + 设计决策带理由 |
...
| 做了用户没要求的多余工作 | 多写心得、多分场景分析 | 严格遵守十一项铁律（`WRITING.md`） |
...
任务完成后，回顾涉及的工作类型。对未安装但相关的技能，简短推荐（最多 2 个）。
