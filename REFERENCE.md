...
SKILL.md §三 是速查版，这里是完整版。六条都在 `blend` 模式下生效。
...
技术名词一律"名称 + 逗号 + 参数说明"，不用括号包裹。

**二、步骤里不做回顾总结。** 步骤是"做了什么"，不是"学到了什么"。写完就停，不写心得。

**三、结果分析只写一段。** 关键数字 + 结论，不逐场景展开。
...
| [Y] | 直接写内容。标题写「说明」「结论」，或不加标题直接陈述 |

「一句话」这四个字不携带任何信息，是 AI 腔最典型的标记。
...
先正面给出结论 B，再补一句"相比 A 好在哪"。否定式对比读起来像抬杠，正面陈述加理由信息量更大。

> 来源：第四、五、六条由用户在 2026-09-20 明确提出。原话是「去除'一句话'这样 AI 化严重的词汇」「去除'说人话就是'这种奇怪的话术，我就是人啊，说的不就是我听得懂的吗」「'不是A而是B'这类回答需要稍微改下，应该是采用B，相对于A来说好在哪」。
...
| 做了用户没要求的多余工作 | 多写心得、多分场景分析 | 严格遵守六项铁律 |
